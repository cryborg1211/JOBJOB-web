...
export default function EmployerReview() {
  ...
  useEffect(() => {
    // Lấy draft từ sessionStorage
    const draft = sessionStorage.getItem('jd_draft')
    if (draft) {
      try {
  ...
  const confirm = async () => {
    try {
      ...
      if (res.ok) {
        // Xóa draft và chuyển đến review với ID
        sessionStorage.removeItem('jd_draft')
        navigate(`/employer/review/${json.id}`)
      } else {
  ...
  return (
    <div className="min-h-dvh bg-[#081A17] text-white">
      ...
      <section className="mx-auto w-full max-w-5xl px-4 sm:px-6 py-12 md:py-16 text-center">
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-extralight tracking-wide">Mô Tả Công Việc Đã Hoàn Tất</h1>
        ...
        <div className="mx-auto mt-10 rounded-[28px] bg-[#1CDAC4] px-6 py-10 sm:px-10 sm:py-12 max-w-2xl shadow-[0_30px_120px_rgba(0,0,0,0.35)]">
          ...
          <div className="mx-auto mb-6 h-24 w-24 overflow-hidden rounded-full ring-4 ring-teal-300/60 shadow bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
            <span className="text-3xl font-bold text-white">
              {data.company ? data.company.charAt(0).toUpperCase() : 'C'}
            </span>
          </div>
