...
function Dropzone() {
  ...
  const setData = useProfile((s)=>s.setData)
  ...
  const onFiles = async (files) => {
    ...
    if (!okExt) { setError('Chỉ nhận PDF, DOC, DOCX'); return }
  ...
  const parse = async () => {
    try {
      ...
      const res = await fetch('http://127.0.0.1:5000/api/parse-resume', { method: 'POST', body: form })
      const json = await res.json()
      if (!res.ok || json.error) { setError(json.error || 'Phân tích thất bại'); return }
      const normalized = {
        ...json,
        languagesText: (json.languages || []).join(', '),
        exp1: (json.experiences || [])[0] || '',
        exp2: (json.experiences || [])[1] || '',
        skill1: (json.skills || [])[0] || '',
        skill2: (json.skills || [])[1] || ''
      }
      setData(normalized)
      ...
      >
        {!file ? (
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-white/80">Kéo thả PDF/DOC/DOCX vào đây hoặc bấm để chọn</p>
            <input ref={inputRef} type="file" accept=".pdf,.doc,.docx" className="hidden" onChange={(e)=>onFiles(e.target.files)} />
          </div>
        ) : (
          <div className="flex flex-col items-center gap-4">
